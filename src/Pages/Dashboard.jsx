  ...
  const Dashboard = () => {
    ...
    const sidebarRef = useRef(null);
      // New state to track if there are new alerts
    ...
    useEffect(() => {
      const fetchUserData = async () => {
        try {
          ...
          const iduser = userData.id; // Use directly instead of fetching from localStorage
          ...
          // Fetch alerts to determine if there are new alerts
          const fetchAlerts = async () => {
            try {
              ...
              // Check if any alert has a non-zero diff
              const newAlerts = alertsData.some(alert => alert.diff.some(num => num !== 0));
  ...
  <NavLink
    ...
    className={({ isActive }) =>
      ...
      } rounded-lg relative` // Added 'relative' for red dot positioning
    }
    onClick={() => {
  ...
  >
    ...
    {item.text}
    {/* Display red dot on sidebar alert icon */}
    {item.to === "alertes" && hasNewAlerts && (
          ...
          <header className={`flex items-center justify-between px-6 py-2 border-b dark:border-gray-700 dark:bg-gray-800 transition-transform duration-300 ${isVisible ? 'transform translate-y-0' : 'transform -translate-y-full'}`}>
            ...
            <div className="flex items-center lg:w-1/3 justify-end space-x-9">
              ...
              <NavLink 
                      ...
                      onClick={() => setHasNewAlerts(false)} // Remove red dot on click
                    >
                      {/* Red dot on navbar alert icon */}
                      {hasNewAlerts && (
